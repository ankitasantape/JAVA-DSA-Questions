	...
	public static int buyTicket(int input[], int k) {
		...
		while ( !pq.isEmpty() ) {
			...
			if( input[i] < pq.element() ) {
				int temp = q.poll(); // remove start index and add it to the last of queue
				q.add( temp );
			
			} else {
				int temp = q.poll(); // remove index of element having highest priority  
				pq.remove(); // remove element at that index
				time++; // increment time to allocate ticket 
