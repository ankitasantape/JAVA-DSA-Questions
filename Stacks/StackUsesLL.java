...
The fourth one is Query-5: It tells whether the stack is empty at this point or not. 
Since the stack has one element and hence it is not empty, false is printed.    */
...
public class StackUsesLL {
	  
	    private Node head;
	 // Agar hum hr bar LL ko traverse krke size nikalenge to O(n) time lagega isliye hum pehle se hi size maintain krne ke lie size variable me store krte rkhte hai j hi push krenge size pluz krenge aur jb bhi pop krenge tb size minus 1 kr denge so hume bar bar traverse nhi krna padega
	    private int size; 
